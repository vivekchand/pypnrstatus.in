...
while True:
    ...
    pnr_notifications = PNRNotification.objects.filter(next_schedule_time__lte=datetime.now()+timedelta(minutes=2))
    print pnr_notifications
    for pnr_notification in pnr_notifications:
        schedule_pnr_notification(pnr_notification)
    pnr_notifications = PNRNotification.objects.filter(next_schedule_time__gte=datetime.now()-timedelta(minutes=2))
    print pnr_notifications
    for pnr_notification in pnr_notifications:
        schedule_pnr_notification(pnr_notification)
